...
export const unfollowUser = (targetId: string, token: string) => async (dispatch: AppDispatch) => {
  try {
    ...
    dispatch({ type: 'UNFOLLOW_SUCCESS', payload: targetId }); // send only ID
  } catch (error: any) {
    const errorMsg = error.response?.data?.message || error.message;
...
export const getFollowersByHandle = (handle: string) => async (dispatch: AppDispatch) => {
  try {
    ...
    dispatch({ type: 'GET_FOLLOWERS_SUCCESS', payload: data }); // reducer converts to IDs
  } catch (error: any) {
    const errorMsg = error.response?.data?.message || error.message;
...
export const getFollowingByHandle = (handle: string) => async (dispatch: AppDispatch) => {
  try {
    ...
    dispatch({ type: 'GET_FOLLOWING_SUCCESS', payload: data }); // reducer converts to IDs
  } catch (error: any) {
    const errorMsg = error.response?.data?.message || error.message;
