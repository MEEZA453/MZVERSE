...
export default function ApproveAttachNotification ({noti}){
    ...
    return <div onClick={()=>router.push(`/${noti?.sender?.handle}`)} className={`h-12 justify-between w-full ${isLightMode ? 'bg-white': 'bg-[#151515]'} flex items-center gap-1 mb-0.5 rounded  px-2`}>
        ...
        <h6 style={{fontWeight : 500}} >@{noti?.sender?.handle} approved your attachment</h6>
        </div>
