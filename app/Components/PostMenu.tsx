...
const handleEditClick = () => {
  ...
  router.push("/createPost"); // ✅ go to form
    ...
    return  <motion.div  initial = {{opacity : 0}} animate = {{opacity : 1}} transition={{duration: 0.3 , }} exit = {{opacity : 0}}  className="h-screen w-screen z-[999] fixed top-0 bg-black/50">
      ...
      <motion.div  initial = {{y : 160 }} transition = {{duration : 0.3 , ease : "easeInOut"} } exit={{y : 160}} animate = {{y : 0}}  className={`${isLightMode ? 'bg-[#ededed]':'bg-[#0d0d0d]'} fixed lg:absolute lg:w-60 lg:top-16 lg:left-[2vw] lg:h-fit  z-200 bottom-4 py-2 max-sm:-translate-x-1/2 max-sm:left-1/2  flex  flex-col items-center justify-center  w-[96%] rounded-[6px]  `}>
