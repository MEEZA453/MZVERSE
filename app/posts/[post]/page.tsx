...
{  assetsOfPost?.length > 0 &&  <Attachments assetsOfPost = {assetsOfPost} setAttachmentsMenu={setAttachmentsMenu} postId={post?._id} token={token}/>}
     <div  onClick={()=>setIsMenu(false)} className={`w-full h-fit lg:border-l -translate-y-24  sticky top-0 z-[100]  lg:mt-24 rounded-t-[10px] lg:rounded-t-none   ${isLightMode ? 'bg-white border-t border-[#dadada]':'bg-black'} lg:h-screen  lg:w-[30vw] `}>
...
{ votes.length > 0 && <div className="flex items-center">
  <h3>Voted by</h3>
  { votes.length > 0 && validVotes.slice(0, 3).map((vote, i) => (
